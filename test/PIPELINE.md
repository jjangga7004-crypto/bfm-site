...
(★v2.1) `opts.roi`(촬영 순간 키포인트 기반 측정 영역)·`opts.exclude`(눈·입술 제외 원)를 받음 — 없으면 중앙 68% 폴백. 카메라 경로는 촬영 버튼 1번에 **연사 3장 → aggregateBurst 중앙값**.
...
| redness | (★2026-09 수정) 느슨한 피부 마스크 위 rg색도 nr가 중앙값+0.03 초과 비율(stride-2 격자) | 0.030 |
| redSpot / redFlush | (★v2.1) 붉은 격자 연결요소 → 점상(≤피부2%)/대면적 분리 — 트러블 축은 redSpot만 | 2% / 0.02% |
...
w_p[i] = w_p · axisRel[i]   # (★v2.1) 축별 사진 신뢰도 [유분1, 건조0.85, 트러블0.95]
F[i] = (1−w_p[i])·Sv[i] + w_p[i]·Sp[i]   # + photo.flush>0.10 → '홍조 주의' 플래그
primary = argmax(F)
