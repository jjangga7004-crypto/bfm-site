# BfM 진단 파이프라인 문서 (2026-09-01 기준, preview 브랜치)

> 대상 파일: `diagnose.html` · `diagnose-bw.html` — **엔진 로직(CAL·VF·fuse·applyRegions·analyzeRegionShot·surveyVector·isSkin·isFaceSkin)은 두 파일이 바이트 단위로 동일**함을 확인(2026-09-01). 수정 시 두 파일 모두 반영해야 함.
> 레거시 face 모드(analyzePhoto/runFace/loopGuide)는 도달 불가 — 실경로는 전부 부위 촬영(tzone→cheek→nose).
...
| shine(광택) | 마스크 픽셀 V값 분위수 문턱 vthr=min(0.96, p60+0.55·(p97−p60)) 초과 비율 | 0.60/0.97/0.55 하드코딩 |
| clipR | V≥0.985 비율 | 하드코딩 |
| redness | r/g 비가 중앙값×1.18 초과인 비율 | 1.18 하드코딩 |
| poreDen | 2px 간격 그리드에서 5×5 이웃평균−자기휘도>14 인 점 비율 | 14 하드코딩 |
...
```
usedPhoto = photo.ok && photo.q ≥ 0.5          # ★ 하드 컷
w_p = usedPhoto ? (regions? 0.45 : 0.30)·q : 0
...
### 명명된 상수 (조정 지점)
- `CAL` — troubleMin 0.28 / shineFull 0.11 / dryZero 0.09 / redOffset 0.02 / redFull 0.13 / poreLo 70 / poreHi 620 / poreDenLo 0.010 / poreDenSpan 0.045 / relOilW 0.55. 주석에 "근거 없음, 손으로 맞춘 값(9회 측정으로 재보정 예정)" 다수.
- `VF` — 위 게이트 표 참조.
...
| 〃 | 0.60/0.97/0.55/0.96 | shine 분위수 문턱 |
| 〃 | 0.985 | 클리핑 판정 |
| 〃 | 1.18 | redness 중앙값 비율 |
| 〃 | 14, 2px, 5×5 | poreDen 국소최소 판정 |
...
### 중복 정의
- 엔진 전체가 diagnose.html·diagnose-bw.html에 **복붙 중복** (현재 동일하지만 한쪽만 고치면 갈라짐).
- regionTheater()의 poreP는 구식 lap 기반(CAL.poreLo/poreHi)을 쓰는데 applyRegions는 poreDen 기반 — **연출과 결과의 모공 숫자가 다를 수 있음**(광택·붉은기는 일치).
- analyzeROI(레거시 face 모드 전용)에도 별도 shine/redness 정의 존재 — 도달 불가라 실해는 없음.
